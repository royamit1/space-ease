    ...
    return (
        <div ref={constraintsRef} className="fixed bottom-0 w-full p-3 my-3 z-10">
            <motion.div
                ...
                className="bg-background min-h-12 rounded-3xl overflow-hidden"
                style={{maxHeight: "calc(100vh - 7rem)"}}
                animate={{height: initialHeight[footerSize]}}
            >
                <div className="h-1.5 p-0.5 w-24 md:w-48 xl:w-96 my-3 mx-auto rounded-full cursor-grab bg-gray-200 dark:bg-gray-700"/>
                <motion.div animate={{height: footerSize === "collapsed" ? "0" : "auto"}} className="overflow-hidden w-full">
                    {children}
                </motion.div>
            </motion.div>
        </div>
